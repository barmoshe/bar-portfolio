...
export default function HeroPitch() {
  ...
  useGSAP(
    () => {
      ...
      const cta = root.querySelector<HTMLElement>('.enter');
  ...
  return (
    <article className="page hero-pitch" id="about" ref={rootRef} tabIndex={-1}>
      ...
      <p className="byline" style={{ fontFamily: 'var(--mono)', color: 'var(--ink-soft)' }}>
        // בר משה — מפתח עצמאי
      </p>
      <h1 className="headline">
        רעיון בראש, <br />
        ואין מושג מאיפה <em style={{ color: 'var(--green)', fontStyle: 'italic' }}>מתחילים?</em>
      </h1>
      <ul
      ...
      <p className="dek" style={{ marginTop: 22 }}>
        זה בדיוק מה שאני עושה — מלמד, מלווה, ובונה. אפליקציות, אתרים, ורעיונות יצירתיים — מהסקיצה
        ועד הלייב.
      </p>
      <a className="enter" href="#contact">
        דברו איתי
      </a>
    </article>
  );
