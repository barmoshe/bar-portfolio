...
export default function Music() {
  ...
  useGSAP(
    () => {
      ...
      const tracks = list ? (Array.from(list.children) as HTMLElement[]) : [];
      ...
      mm.add(FULL_MOTION_QUERY, () => {
        ...
        if (rig) {
          // Rig fades in + tips slightly forward, then flips data-playing
          // which both starts the disc and swings the tonearm over.
        ...
        if (tracks.length) {
          gsap.set(tracks, { opacity: 0, y: 30 });
          gsap.to(tracks, {
            opacity: 1,
            y: 0,
            duration: 0.7,
...
const Rig = ({ ref }: { ref: React.RefObject<HTMLDivElement | null> }) => (
  <div className="rig" ref={ref} data-playing="false" aria-hidden="true">
    <svg viewBox="0 0 240 240" role="img" aria-label="Spinning vinyl on a turntable">
      ...
      {/* The disc + grooves rotate together. Everything mounted OFF the disc
          (tonearm, spindle, sheen) stays static. */}
      <g className="disc">
        ...
        {/* Label text — wraps the spindle on two arcs so it reads as printed
            sleeve art when the disc is at rest and blurs into a solid
            ring once it's spinning. */}
        <defs>
          <path
            id="music-label-top"
