...
7. **Accessibility settings panel (Phase 2)** - the ⚙ button in `Strip` opens `src/components/AccessibilityPanel.tsx`. Reads/writes `bm:contrast`, `bm:text-scale`, `bm:readable` via `src/hooks/usePreferences.ts`. The inline pre-paint script in `index.html` mirrors these reads to avoid FOUC; the script and the hook must stay in sync if you add a new pref. **Do not** install a third-party accessibility overlay widget (UserWay, accessiBe) - they harm disabled users and invite lawsuits. **HeroSlides has a keyboard-accessible Pause button** (`src/components/HeroSlides.tsx`) - WCAG 2.2.2 Level A. **Boot has a focus trap** - keyboard users cannot escape the cover before dismissing it.
