...
- **Vite 6** for dev server, build, and preview. Multi-entry: `index.html` (portfolio) + `business/index.html` (bilingual marketing site under `src/marketing/` - Hebrew default with an English toggle, dictionaries in `src/marketing/i18n.ts`, language persisted in `bm:marketing-lang`).
